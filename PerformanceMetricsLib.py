...
def calc_TPAndFP_Mod(ref, hyp, toleranceFP_bef, toleranceFP_aft):
    ...
    #     ref:       |       <--------------------->
    #     hyp:     ----------------
    if (start_hyp <= start_ref  and stop_hyp > start_ref -toleranceFP_bef ): #+ tolerance
        tp = 1
    #     ref:              <--------------------->   |
    #     hyp:                         <----------------
    elif (start_hyp < stop_ref + toleranceFP_aft  and  stop_hyp >= stop_ref ): #- tolerance
        tp = 1
    #     ref:              <--------------------->
    #     hyp:                         <---->
...
def performance_episodes(predLab, trueLab,  toleranceFP_bef, toleranceFP_aft):
    '''
    function that detects episodes in a stream of true labels and predictions
    ...
    if (len(trueEvents)==0):
        ...
        for etIndx, eTrue  in enumerate(trueEvents):
            for epIndx, ePred in enumerate(predEvents):
                (tp0, fp0, fp_bef, fp_aft)= calc_TPAndFP_Mod(eTrue, ePred,  toleranceFP_bef, toleranceFP_aft)
...
def smoothenLabels_Bayes(prediction,  probability, seizureStableLenToTestIndx,  probThresh, distanceBetweenSeizuresIndx):
    ...
    #'probability' has the probability of predicted class, but Bayes needs the probability per target
    probability_pos=np.copy(probability)
    indxs=np.where(prediction==0)[0]
    probability_pos[indxs]=1-probability[indxs] #keeps only the target '1''s probability

    conf_pos=np.zeros((len(probability)))
    conf_neg=np.zeros((len(probability)))
    ...
    #first classifying as true 1 if at laest  GeneralParams.seizureStableLenToTest in a row is 1
    conf_pos[0:seizureStableLenToTestIndx] = probability_pos[0:seizureStableLenToTestIndx]
    conf_neg[0:seizureStableLenToTestIndx] = 1-probability_pos[0:seizureStableLenToTestIndx]
    for i in range(int(seizureStableLenToTestIndx), int(len(prediction))):
        probThisWind=probability_pos[i-seizureStableLenToTestIndx: i]
        conf_pos[i]=np.prod(probThisWind)
        conf_neg[i]=np.prod( 1-probThisWind)
        
    conf=np.log( (conf_pos+ 0.00000001) /(conf_neg + 0.00000001))
    smoothLabels = conf > probThresh
    #second part: merging detections that are too close into one
    smoothLabelsStep2 = np.copy(smoothLabels)
    prevSeizureEnd=-distanceBetweenSeizuresIndx
    for i in range(1,len(prediction)):
        if (smoothLabelsStep2[i] == 1 and smoothLabelsStep2[i-1] == 0):  # new seizure started
            # find end of the seizure
    ...
    return  (smoothLabelsStep2)



def calculatePerformanceAfterVariousSmoothing(predLabels, label, probabilityLabels,toleranceFP_bef, toleranceFP_aft, numLabelsPerHour, seizureStableLenToTestIndx, seizureStablePercToTest,  distanceBetweenSeizuresIndx, bayesWind, probThresh):
    ...
    performancesAll[2*numPerf: 3*numPerf]= performance_all9(yPred_SmoothOurStep1, label, toleranceFP_bef, toleranceFP_aft,  numLabelsPerHour)

    #bayes smoothing
    yPred_SmoothBayes=smoothenLabels_Bayes(predLabels, probabilityLabels, bayesWind, probThresh, distanceBetweenSeizuresIndx)
